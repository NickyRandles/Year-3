...
// function searchSuggest
	...
	// Get the counties file async
		xhr.open("GET", "http://localhost:8080/Lab04/question03/counties.json", true);
...
function setStatesArray() {
	if (xhr.readyState == 4) {
		if (xhr.status == 200) {
			...
			var allStates=jsondata.counties;
		...
		// get tags with item as tag name and store in allStates array
			...
			// store the text value of the label tags in the statesArray, ie the state name
				statesArray[i] = jsondata.counties[i].label;
			}
